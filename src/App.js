...
class App extends Component {
  ...
  onClickChechBtn = id => {
    ...
    const imageId = imagesList.filter(
      eachItem => eachItem.imageUrl === imageUrl,
    )
    if (imageId[0].id === id) {
      const random =
      ...
      clearInterval(this.timerId)
      this.setState({gameResult: true, time: 60})
    }
  }

  onClickResetBtn = () => {
    this.setState({score: 0, time: 60, gameResult: false})
    this.timerId = setInterval(this.rendertimer, 1000)
  }

  render() {
    const {activeTab, imageUrl, gameResult, score, time} = this.state
      ...
      <div className="bgContainer">
        ...
        {gameResult === true ? (
          <div className="ScoreCardContainer">
            ...
            />
            <p className="ScoreCardtext1">Your Score</p>
            <p className="ScoreCardtext2">{score}</p>
            <button
              type="button"
              className="resetBtn"
            ...
            >
              <img
                ...
                alt="reset"
                className="resetBtnImage"
              />
              <p className="ScoreCardplayagain">Play Again</p>
            </button>
          </div>
        ) : (
          <div className="gameCardContainer">
            <img src={imageUrl} alt="match" className="randomImage" />
            ...
            <ul className="imagesListContainer">
              {updatedImagesList.map(eachItem => (
                <ImagesList
                  imagesList={eachItem}
                  onClickChechBtn={this.onClickChechBtn}
                  key={eachItem.id}
                />
              ))}
            </ul>
